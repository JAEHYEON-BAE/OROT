...
from vinyl_core.adapters.base import MIN_CRAWL_INTERVAL_SECONDS, SourceAdapter
...
def load_adapters(*, force: bool = False) -> None:
    ...
    package = importlib.import_module("vinyl_core.adapters")
    for module in pkgutil.iter_modules(package.__path__):
        if module.name in _NON_ADAPTER_MODULES or module.name.startswith("_"):
            continue
        importlib.import_module(f"vinyl_core.adapters.{module.name}")
