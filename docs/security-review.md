...
### 2026-09-11 추가 (T-116 검증 중 발견)

| 문제 | 영향 | 수정 |
|---|---|---|
| `Settings` 의 비밀이 `repr` 에 노출 | 트레이스백 렌더러가 지역 변수를 `repr()` 로 찍는다. 스케줄러 주기 실패 시 운영자 키·VAPID 개인키·DB 암호·Slack 웹훅 URL 이 컨테이너 로그에 전부 남았다. 로컬에서는 로그가 호스트에 머물지만, 로그 수집 환경으로 옮기면 사고가 된다 | 해당 4개 필드에 `Field(repr=False)`. 값은 그대로 읽히고 `repr` 에서만 빠진다. 비밀이 아닌 설정은 디버깅을 위해 그대로 둔다 |
...
- 현재 프로젝트 venv의 설치 패키지 69개를 임시 `pip-audit`로 검사: 알려진 취약점
  **0건**. 프로젝트 자체인 `orot-*` 3개는 공개 패키지 DB 검사에서 제외했다.
