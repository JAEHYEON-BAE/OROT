...
import httpx
import typer
from orot_core import __version__ as core_version
from orot_core.adapters import get_adapter
from orot_core.alerts import Alert
from orot_core.db import session_scope
from orot_core.enums import DevicePlatform, EventType
from orot_core.logging import configure_logging
from orot_core.models import DeviceToken, ListingEvent, Release
from orot_core.notifications import SendOutcome, build_payload
from orot_core.seed import seed_sources
from orot_core.settings import get_settings
from orot_core.testing import ACCEPTED
from sqlalchemy import select

from orot_collector.bridge import FetcherPageAdapter
from orot_collector.fetcher import Fetcher, FetchOutcome, SourceBlockedError
from orot_collector.push_sender import WebPushSender
from orot_collector.scheduler import run as run_scheduler
from orot_collector.slack_alerter import SlackAlerter

app = typer.Typer(help="OROT 수집기", no_args_is_help=True)
...
def version() -> None:
    ...
    typer.echo(f"orot-collector {core_version}")
...
def test_push(
    ...
    **전송(VAPID·키·네트워크)** 인지 **로직(이벤트 생성·발송 대상 선정)** 인지
    ...
    `--case` 는 `orot_core.testing` 의 카탈로그를 그대로 쓴다. 이모지·긴 제목·
...
def _release_for(name: str) -> tuple[Release, str | None]:
    ...
    # 카탈로그의 원본 입력을 그대로 쓴다. 검증(`ReleaseIn`)은 `orot_api` 에 있고
