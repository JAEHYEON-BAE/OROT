...
어댑터는 `packages/core` 에 있어 `orot_collector.fetcher` 를 임포트할 수 없다(의존 방향).
그래서 core 는 최소 프로토콜만 선언하고, 실제 연결은 여기서 한다.
"""

import structlog

from orot_collector.fetcher import Fetcher, FetchOutcome

log = structlog.get_logger(__name__)


class FetcherPageAdapter:
    """`Fetcher` 를 `orot_core.adapters.base.PageFetcher` 로 감싼다.
