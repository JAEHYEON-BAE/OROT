import { getFeed, getRelease, sellerUrl, validRelease } from '../src/lib/api/client';
...
test('sort is requested from the real service contract', async () => {
  ...
  expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining('/api/mobile/v1/feed?limit=100&sort=recent'), expect.any(Object));
});
...
test('server errors keep status, and non-JSON is rejected', async () => {
  ...
  (global.fetch as jest.Mock).mockResolvedValue({ ok: true, headers: { get: () => 'text/html' } });
  await expect(getFeed('recent')).rejects.toThrow('응답');
});
test('aborts stalled requests instead of loading forever', async () => {
  jest.useFakeTimers();
  (global.fetch as jest.Mock).mockImplementation((_url, { signal }) => new Promise((_resolve, reject) => signal.addEventListener('abort', () => reject(new Error('aborted')))));
  const result = expect(getFeed('recent')).rejects.toThrow('네트워크');
  await jest.advanceTimersByTimeAsync(12000); await result; jest.useRealTimers();
});
test('seller links only allow HTTP(S) without credentials', () => {
  ...
  expect(sellerUrl('https://example.com/record')).toBe('https://example.com/record');
});
