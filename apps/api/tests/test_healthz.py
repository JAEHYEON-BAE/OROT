...
DB 는 실물 대신 `orot_core.db.ping` 을 대체하여 두 경로를 모두 검증한다.
...
def test_healthz_returns_200_when_database_reachable(
    ...
    monkeypatch.setattr("orot_api.main.ping", _ok)
...
def test_healthz_returns_503_when_database_unreachable(
    ...
    monkeypatch.setattr("orot_api.main.ping", _fail)
