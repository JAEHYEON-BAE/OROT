...
from orot_api.feed_identity import FEED_ID_NAMESPACE
from orot_api.icalendar import CalendarBuilder

router = APIRouter(prefix="/v1", tags=["calendar"])

CALENDAR_NAME = "OROT — 발매·예약 일정"
...
def _uid(release_id: int, kind: str) -> str:
    ...
    return f"{kind}-{release_id}@{FEED_ID_NAMESPACE}"
...
async def releases_ics(
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": 'inline; filename="orot.ics"',
